...
class BillerCode741 implements BillerCdvInterface
{
    ...
    private function validateOldAlgoFormat($mainField) {
        ...
        $firstCondition = [11, 12, 13, 14, 15, 16, 33];
        $secondCondition = [31, 32, 41];

        if (
            (in_array($twoDigit, $firstCondition) && $threeDigit >= 1 && $threeDigit <= 999) ||
            (in_array($twoDigit, $secondCondition) && $threeDigit == 000)
        ) {
            $lastDigit = substr($mainField, -1);
